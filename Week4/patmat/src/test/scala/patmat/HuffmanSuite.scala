...
class HuffmanSuite {
  ...
  @Test def `QuickEncode Encode Text into List[Bit]`: Unit = {
    ...
    val toEncode = string2Chars("ladernierefonctiondututo")
    println(quickEncode(frenchCode)(toEncode))
    assertEquals(toEncode, decode(frenchCode, quickEncode(frenchCode)(toEncode)))
  }
